    ...
    public int strategy() { //remember not change queue and change direction
        ...
        synchronized (this.queue) {
            if (personRequests.size() != 0 && personRequests.size() != 6) {
                ...
                PersonRequest min = queue.get(0);
                for (PersonRequest i: queue) {
                    if (abs(i.getFromFloor() - floor) < abs(min.getFromFloor() - floor)) {
                        min = i;
                    }
                }
                if (min.getFromFloor() - floor > 0) {
                    direction = 1;
                }
                else if (min.getFromFloor() - floor == 0) {
                    if (min.getToFloor() - min.getFromFloor() > 0) {
                        direction = 1;
                    }
                    else {
                        direction = -1;
